...
- `test_event_latent_roundtrip_and_high_projection` loads the ledger, verifies its persisted SHA, and defines `rain_latent_coverage(instance)` locally in the test module (or imports that explicitly named Task 2.5 helper). It requires base status/disruptions to be byte/hash-equal to the persisted projection; `non_rain_base_ids ⊆ non_rain_high_ids` (high may append approved document/base-failure candidates), forced and priority rows to be exact across base/high, and high rain rows to cover exactly the active immutable block latent IDs under the canonical coalescing algorithm with the canonical recoalesced overlay hash.
