...
Cada evento em `disruptions.jsonl` referencia `latent_id` e `event_origin`, permitindo conferir a projeção. Para eventos documentais e falhas-base, `latent_id` é o ID do candidato do ledger. Para chuva, uma linha coalescida usa `latent_id=f"{instance_id}:rain_coalesce:{first_block:02d}-{last_block:02d}"`; o intervalo é a união de blocos adjacentes ativos e `payload_hash` cobre a lista ordenada de IDs de blocos, de modo que a relação muitos-para-um seja reconstituível sem supor uma linha por bloco. Na intensidade `base` (`multiplier=1,00`), o derivador reproduz a projeção persistida: `document_status` e as linhas coalescidas de `disruptions` permanecem byte a byte e hash a hash. Na intensidade `high`, usa-se somente `u < min(1, 2p)` para bloqueio documental, falha-base e cada bloco de chuva; todos os eventos aprovados formam um fluxo canônico novo e contíguo, com `sequence` renumerada e `payload_hash` recomputado por linha para a projeção. Para eventos presentes em ambas as projeções, a igualdade obrigatória é semântica nos campos `event_type`, `latent_id`, `event_origin`, `resource_id`, `truck_id`, `cause`, `operation`, `time`, `duration_min` e `return_time`; `sequence` e `payload_hash` são próprios da projeção. Eventos documentais/falhas-base novos são anexados à seleção, e os blocos de chuva aprovados são então coalescidos novamente pelo mesmo algoritmo canônico; por isso suas linhas e `latent_id` de intervalo podem diferir, embora blocos, recursos, inícios e durações candidatos permaneçam imutáveis. Nenhuma duração ou evento `forced` é recalculado, removido ou alterado. `controlled_view_hash` e `event_overlay_hash` são atestações distintas da identidade da projeção. O loader rejeita o dataset antigo de cinco payloads, ledger ausente, corrompido, não canônico, com linha faltante/extra ou hash divergente antes de fornecer uma instância.
...
`derive_controlled_instance(instance, event_latents, controls) -> FrozenInstance` é uma função pura: recebe somente uma `FrozenInstance`, o ledger carregado e um controle imutável com hashes de origem; nunca usa RNG, relógio, filesystem ou `ExperimentConfig` mutável. Para `controls.intensity='base'` e `threshold_multiplier=Decimal('1.00')`, ela retorna a projeção de status/eventos byte/hash-equal à persistida, inclusive as linhas de chuva coalescidas e seus hashes. Para `high`, aplica `u < min(1, 2p)` apenas às três taxas Bernoulli autorizadas; os eventos aprovados são serializados em um fluxo canônico novo e contíguo, com `sequence` renumerada e `payload_hash` recomputado por linha. Em eventos presentes nas duas projeções, exige igualdade semântica em `event_type`, `latent_id`, `event_origin`, `resource_id`, `truck_id`, `cause`, `operation`, `time`, `duration_min` e `return_time`; `sequence` e `payload_hash` não são invariantes entre projeções. Eventos documentais/falhas-base novos são anexados e os blocos de chuva são coalescidos novamente, podendo alterar linhas/IDs de intervalo; `controlled_view_hash` e `event_overlay_hash` permanecem atestações distintas da identidade da projeção. Falha crítica forçada, prioridades, chegadas, serviços e candidatos de duração já realizados não mudam; `scheduled_failure_start`/`effective_failure_start` são apenas registrados deterministicamente. Ledger antigo, ausente ou corrompido, hash de origem divergente, controle com campo faltante ou tentativa de alterar `instance`/`config` falham antes do DES.
